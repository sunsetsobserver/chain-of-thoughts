...
def prompt_bars456_pcsets_contract(b1_scale: dict, harm2: dict, bar3: dict) -> Tuple[str, str]:
    """
    Ask for *simultaneities* only (no tonal system requirement). The model returns pitch-class sets (0..11)
    and simple voicing/register hints for:
      - Bar 4 (3/4): one chord filling the bar
      - Bar 5 (4/4): one chord filling the bar
      - Bar 6 (4/4): two chords, each half-bar

    We pass a compact reminder of Bars 1–3 (scale used in bar1, bar2 plan summary, bar3 contrast meta)
    *only as context*.
    """
    b1_name = b1_scale.get("name", "(unknown)")
    b1_tonic = b1_scale.get("tonic_midi", "?")
    b1_pcs = b1_scale.get("pcs", [])
    ...
    system = """
You output ONLY strict JSON. No prose.
Return pitch-class sets (0..11) for simultaneities, plus voicing/register hints per instrument.
Do NOT encode using scale degrees. Use absolute pitch-classes (C=0, C#=1, ... B=11).
Schema:
{
  "bar4": {
    "pcset": [int...],                        // unordered set, size 3..12
    "voicing_hint": "cluster|spread",
    "register_targets": {
      "alto_flute": "low|mid|high",
      "violin": "low|mid|high",
      "bass_clarinet": "low|mid|high",
      "trumpet": "low|mid|high",
      "cello": "low|mid|high",
      "double_bass": "low|mid|high"
    },
    "avoid_unisons": "mild|strong"
  },
  "bar5": { /* same shape */ },
  "bar6": [
    { /* first half, same shape */ },
    { /* second half, same shape */ }
  ]
}
Constraints:
- pcset size 3..12.
- Hints are artistic; the program will realize actual pitches per instrument range.
"""
    user = f"""
Previous context (for color only):
- Bar1 scale: name={b1_name}, tonic_midi={b1_tonic}, pcs={b1_pcs}
- Bar2 plan (degree_roots/chord_kinds/inversions): {bar2_summary}
- Bar3 contrast (name/tonic/pcs and chosen): name={contrast.get('name')}, tonic={contrast.get('tonic_midi')}, pcs={contrast.get('pcs')}, chosen={chosen}

Now propose:
- Bar4 (3/4, ticks 36..47): one pcset chord + hints.
- Bar5 (4/4, ticks 48..63): one pcset chord + hints.
- Bar6 (4/4, ticks 64..79): two pcset chords (first half 64..71, second half 72..79) + hints each.
Return valid JSON only.
"""
    return system.strip(), user.strip()

def prompt_bars789_clouds_contract(b1_scale: dict, harm2: dict, bar3: dict, pc456: dict) -> Tuple[str, str]:
    """
    Ask for:
      - one continuation pc-set ("scale": name/tonic_midi/pcs as before)
      - one style flag "long" | "staccato"
      - three disjoint instrument pairs covering all 6 instruments
      - for each of Bars 7–9 (3/4, 12 ticks), denser, polyphonic "cloud" writing for the active pair:
        interlocking rhythms, melodic motion, and counterpoint (not block chords).
    """
    b1_name = b1_scale.get("name","(unknown)")
    b1_tonic = b1_scale.get("tonic_midi","?")
    b1_pcs = b1_scale.get("pcs",[])
    ...
    bar2_summary = {
        "degree_roots": harm2.get("degree_roots", []),
        "chord_kinds":  harm2.get("chord_kinds", []),
        "inversions":   harm2.get("inversions", []),
    }
    chosen = bar3.get("chosen_instruments", [])
    contrast = bar3.get("contrast_scale", {})

    def _brief(pcdef):
        if not isinstance(pcdef, dict): return {}
        return {
            "pcset": pcdef.get("pcset", []),
            "voicing_hint": pcdef.get("voicing_hint", ""),
            "avoid_unisons": pcdef.get("avoid_unisons", "")
        }
    b46 = {
        "bar4": _brief(pc456.get("bar4", {})),
        "bar5": _brief(pc456.get("bar5", {})),
        "bar6": [ _brief(pc456.get("bar6",[{}])[0]) if isinstance(pc456.get("bar6"), list) and len(pc456["bar6"])>0 else {},
                  _brief(pc456.get("bar6",[{},{}])[1]) if isinstance(pc456.get("bar6"), list) and len(pc456["bar6"])>1 else {}]
    }

    system = """
You output ONLY strict JSON. No prose.

Task: Continue with three 3/4 bars (each 12 ticks, 1/16 grid) as *polyphonic clouds* where each bar uses a DIFFERENT PAIR of instruments.
Decide a global style ("long" = sustained but still active; "staccato" = short, lively). Write **melodic/rhythmic material** for
both instruments in the active pair so they create *independent, interlocking lines* (not block chords).

Return:
{
  "scale": {"name":"string","tonic_midi":int,"pcs":[int 0..11 ...]},  // ordered PC set for pitch realization
  "style": "long|staccato",
  "pairs": [["instrA","instrB"], ["instrC","instrD"], ["instrE","instrF"]],  // disjoint, cover all 6
  "bars": [
    {
      "pair_index": 0,  // Bar 7
      "rhythm": { "<instr>": {"time":[0..11 inc, strictly inc], "duration":[>0, same len]} , ... },
...
Polyphony/Activity rules (apply to EACH active instrument in its bar):
- Note density:
  - If "staccato": 6–10 onsets per instrument (durations 1–2), with clear gaps.
  - If "long": 4–7 onsets per instrument (durations mostly 3–6, occasional tie-like long 7–10).
- Interlocking: avoid aligning onsets more than ~1/3 of the time; favor call–response, hocket, and off-beat placement.
- Motion variety: use a mix of stepwise and small skips; avoid parallel motion for more than two consecutive onsets.
- Contour: give the two lines contrasting contours (one rising while the other falls; or wave vs. terraced).
- Register: keep a modest register separation between the pair (don’t sit on the same pitch area the whole time).
- Monophony: times strictly increasing per instrument; time[i]+duration[i] <= 12.
- Arrays must be valid: lengths match; durations positive integers.

"pcs" is an ORDERED pitch-class set; "steps" are SCALE-DEGREE steps within that order (can be negative).
"""

    user = f"""
Context summary:
- Bar1 scale: name={b1_name}, tonic={b1_tonic}, pcs={b1_pcs}
- Bar2 plan: {bar2_summary}
- Bar3 contrast: name={contrast.get('name')}, tonic={contrast.get('tonic_midi')}, pcs={contrast.get('pcs')}, chosen={chosen}
- Bars4-6 pc/hints: {b46}

Instruments universe: ['alto_flute','violin','bass_clarinet','trumpet','cello','double_bass'].
...
def realize_bars789_clouds(plan: dict) -> Dict[str, Dict[str, List[int]]]:
    """
    Realize Bars 7–9 from the LLM cloud plan.
    Uses a single continuation scale in plan["scale"] for pitch realization.
    Style affects nothing here beyond trusting durations already decided by LLM.
    """
    out = {instr: {"time": [], "duration": [], "pitch": [], "velocity": []} for instr in ORDERED_INSTRS}

    scale = plan.get("scale", {})
    if not (isinstance(scale.get("pcs", []), list) and isinstance(scale.get("tonic_midi", 0), int)):
        # fallback to bar1-like neutral if something is wrong
        scale = {"name":"cont","tonic_midi": 60, "pcs": [0,2,3,6,7,10]}
    ...
    for bi in range(min(3, len(bars))):
        bar = bars[bi]
        pi = int(bar.get("pair_index", bi))  # default: 0,1,2
        ...
        for instr in active:
            ...
            if isinstance(steps, list) and len(steps) == max(0, len(t_local) - 1):
                p = pitches_from_steps(t_local, steps, int(scale["tonic_midi"]), scale["pcs"], rng, tess, start_hint)
            else:
                p = scale_walk_assign_pitches(t_local, int(scale["tonic_midi"]), scale["pcs"], rng, tess)

            v = [VEL_BAR3_PP for _ in t_local]  # keep soft
            out[instr]["time"]    += t_abs
            out[instr]["duration"]+= d_local
            out[instr]["pitch"]   += p
            out[instr]["velocity"]+= v

        # everyone else silent in this bar (no-op; padding happens later)
    return out
...
def realize_pcset_event(pcset: List[int], voicing_hint: str, reg_targets: Dict[str,str], avoid_unisons: str,
                        prev_pitches: Dict[str,int] = None) -> Dict[str,int]:
    """
    Choose one pitch per instrument whose pitch-class is in pcset, using range/tessitura and hints.
    prev_pitches: optional for smooth voice-leading (minimize moves).
    Returns: {instr: pitch}
    """
    out = {}
    taken = set()
    for instr in ORDERED_INSTRS:
        ...
        if prev_pitches and instr in prev_pitches and (prev_pitches[instr] % 12) in pcset:
            ...
            # choose a pc that doesn't collide too much with already taken (if 'strong')
            options = list(pcset)
            if avoid_unisons == "strong" and taken:
                # prefer a pc not yet used; otherwise fallback
                unused = [pc for pc in options if pc not in taken]
                if unused: options = unused
            # simple deterministic pick by seat order
            target_pc = options[0] if options else (approx % 12)

        p = _nearest_pitch_for_pcset(target_pc, approx, (lo, hi))
        # if still colliding exactly and avoid strong, try another pc
        if avoid_unisons == "strong" and p in out.values() and len(pcset) > 1:
            for alt_pc in pcset:
                if alt_pc == target_pc: continue
                q = _nearest_pitch_for_pcset(alt_pc, approx, (lo, hi))
                if q not in out.values():
                    p = q; target_pc = alt_pc; break
...
def realize_bars456(pc_plan: dict) -> Dict[str, Dict[str, List[int]]]:
    """
    pc_plan is the JSON from prompt_bars456_pcsets_contract.
    Returns per-instrument dicts with absolute time/duration/pitch/velocity for bars 4–6.
    Bar4: one chord filling 3/4 (12 ticks) at BAR4_OFF
    Bar5: one chord filling 4/4 (16 ticks) at BAR5_OFF
    Bar6: two chords half-bar (8 ticks each) at BAR6_OFF and BAR6_OFF+8
    Dynamics: pp (VEL_BAR3_PP)
    """
    out = {instr: {"time": [], "duration": [], "pitch": [], "velocity": []} for instr in ORDERED_INSTRS}
    ...
    def append_simul(abs_time: int, dur: int, pcdef: dict, prev_pitches: Dict[str,int] = None):
        ...
        picks = realize_pcset_event(pcset, voh, regs, au, prev_pitches)
...
def main():
    ...
    sysC, usrC = prompt_bars456_pcsets_contract(b1_scale, harm2, bar3)
    seedC = random.randint(1, 10**9)
    usrC += f"\nCreativeSeed: {seedC}\nRule: When several valid pcsets/hints fit, bias choices using CreativeSeed for diversity."
    pc456 = call_llm_json(oai, sysC, usrC, model="gpt-4.1", temperature=0.8)
    ...
    sysD, usrD = prompt_bars789_clouds_contract(b1_scale, harm2, bar3, pc456)
    seedD = random.randint(1, 10**9)
    usrD += f"\nCreativeSeed: {seedD}\nRule: Use CreativeSeed to break ties (pairs selection and rhythms)."
    cloud789 = call_llm_json(oai, sysD, usrD, model="gpt-4.1", temperature=0.8)
